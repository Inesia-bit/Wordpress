	...
	class GhostPool_Menu_Item_Custom_Fields {
		...
		*/
		public static function load() {
			global $wp_version;
			if ( version_compare( preg_replace( "/[^0-9\.]/", "", $wp_version ), strtolower( '5.4' ), '<' ) ) {
...
class Ghostpool_Menu_Item_Custom_Fields_Options {
	...
	public static function init() {
		...
		self::$fields = array(
			...
			'gp-icon' => esc_html__( 'Icon', 'socialize' ),
			...
			?>
				...
				<?php if ( $_key == 'gp-icon' ) { ?>
					<p class="description description-wide <?php echo esc_attr( $class ) ?>">
						...
						<br/><span class="description"><?php esc_html_e( 'Add the class name of an icon from', 'socialize' ); ?> <a href="https://fontawesome.com/v4.7.0/cheatsheet/" target="_blank"><?php esc_html_e( 'here', 'socialize' ); ?></a> <?php esc_html_e( 'e.g. fa-envelope', 'socialize' ); ?></span>
					</p>		
				<?php } ?>
				...
				<?php if ( $_key == 'gp-content' && $depth >= 1 ) { ?>
					<p class="description description-wide <?php echo esc_attr( $class ) ?>">
						...
						<br/><select id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $name ); ?>">
							...
							<option value="gp-menu-header"<?php if ( $value == 'gp-menu-header' ) { echo 'selected'; } ?>><?php esc_html_e( 'Menu Header', 'socialize' ); ?></option>
						</select>		
